...
namespace AkkaSync.Host.Application.Swapping
{
  public static class PluginStateReducer
  {
    ...
    {
      PluginManagerInitialized e => current with { Entries = [..e.Plugins.Select(p => new PluginCacheInfo(p.Id, p.Version, PluginStatus.Loaded))] },
      PluginAdded e => current with { Entries = [ ..current.Entries.Select(entry => entry.Id == e.Name ? entry with { Status = PluginStatus.Loaded } : entry)] },
